...
if __name__ == '__main__':
    data_dir = args.dir
    print('Train on dataset', data_dir, 'CUDA', args.cuda)
    import json 
    ...
    for c in range(len(config['chrom_list'])):
        ...
        train_set = GraphSet(cells, neibor_limit, train_pos)
        ...
        test_loader = DataLoader(test_set, config['batch_size'], shuffle = True, num_workers = config['num_workers'], drop_last = True )

        negset = NegSet(cells, config['neg_num'])
        negloader = DataLoader(negset, 32, shuffle = False, num_workers = 16, collate_fn = lambda d: d)
        
        state = 'pretrained' if args.pre else 'init'
        model = get_model(data_dir, c, state, 'cuda')
        loss_func = nn.BCELoss()
        opt = th.optim.Adam(model.parameters(), lr = 1e-3, weight_decay = 1e-5)
        
        if args.pre:
            params = [ {'params': model.sub_encoder.parameters(), 'lr': 1e-4},
                    {'params': model.cell_encoder.parameters(), 'lr': 1e-3}, 
                    {'params': model.decoder.parameters(), 'lr': 1e-3} ]
            opt = th.optim.Adam(params, lr = 1e-3, weight_decay = 1e-5)
        
        epoch_num = config['train_epoch']
        start_epoch = 0
        device = 'cuda'
        save_interval = 1
        val_interval = 1
        ...
        for epoch in range(start_epoch, start_epoch + epoch_num):
            ...
            train_epoch(model, opt, loss_func, train_loader, device, test_loader, epoch, val_interval, True)
            if epoch % save_interval == 0:
                th.save(model.state_dict(), os.path.join(weight_path_c, str(epoch)))